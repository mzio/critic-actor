...
from .grader import HumanitiesLastExamEval
...
class HLEEnv(Environment):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.grader_model = HumanitiesLastExamEval(grader_model_config=grader_model_config)
        ...
        for action_idx, action in enumerate(parsed_actions):
            ...
            if action.type in ["message", "reasoning"]:
                ...
                if (
                    ...
                    user_content = (
                        "Correct!" if reward == 1 else ("Incorrect! " + grader_text).replace("  ", " ")
                    )
                    messages.append({
                        "role": "user",
    ...
    def _init_data(self) -> dict[str, HFDataset]:
        ...
        """
        dataset = load_dataset(**self.dataset_config)["test"]
        dataset = dataset.filter(lambda x: x["image"] == "")  # no images for now
        datasets = dataset.train_test_split(
            test_size=self.val_split_size, 
            seed=self.seed
        )
        # Split so eval and test are both self.val_split_size
        train_eval_size = 1 - self.val_split_size
        dataset_train_eval = datasets["train"].train_test_split(
            test_size=self.val_split_size / train_eval_size, 
            seed=self.seed
        )
        datasets = {
            "train": dataset_train_eval["train"],
            "eval": dataset_train_eval["test"],
            "test": datasets["test"]
        }
    ...
    def _preprocess_data_fn(self, sample: dict[str,any], sample_idx: int) -> dict[str,any]:
        """
        Preprocess individual data samples into multiple-choice questions.
        """
        question = sample[self.prompt_column]
        answer = sample[self.answer_column]
        return {
            "system_prompt": self.system_prompt,
            "prompt": question,
            "answer": answer,
